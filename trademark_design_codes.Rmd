...
molten_filtered_totals <- pivot_longer(select(case_file_filtered_totals, year, app_total, pub_total), cols = c('app_total', 'pub_total'), names_to = 'variable', values_to = 'total')
...
case_file_filtered_nonword_totals <- case_file_nonword_filtered |>
  ...
  select(year, app_total_nonword, pub_total_nonword) |>
  filter(!duplicated(year))

case_file_filtered_nonword_percentages <- case_file_filtered_nonword_totals |>
  mutate(percentage = pub_total_nonword/pub_total_nonword)
...
figure_2 <- ggplot(molten_filtered_nonword_totals, aes(year, value, fill = variable)) +
  geom_col(position = 'dodge') +
  labs(x = 'Year', y = 'Number') +
  scale_fill_manual(name = '', labels = c('Publications', 'Applications'), values = c('#099999', '#203769')) +
  scale_x_continuous(breaks = seq(1985, 2025, by = 5), limits = c(1986, 2023), expand = c(0, 0)) +
