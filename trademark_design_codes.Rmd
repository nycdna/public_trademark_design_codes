...
regression_df <- df_no_codes |>
  ...
  group_by(serial_no) |>
  mutate(across(c('n', 'registered', 'published', all_of(intl_classes), 'bin19861990', 'bin19911995', 'bin19962000', 'bin20012005', 'bin20062010', 'bin20112015', 'bin20162020', all_of(years), 'attorney'), max)) |>
  filter(!duplicated(serial_no)) |>
  ungroup()
...
regression_df_1 <- regression_df |>
  ...
  select(highcode, published, all_of(intl_classes), bin19861990, bin19911995, bin19962000, bin20012005, bin20062010, bin20112015, bin20162020, attorney)
  
mod1 <- glm(published ~ ., family = binomial, data = regression_df_1)
