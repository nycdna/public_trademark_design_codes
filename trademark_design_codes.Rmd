...
alt_regression_df_2 <- df_no_codes |>
  ...
  group_by(serial_no) |>
  summarise_at(c('n', intl_classes, years, 'attorney'), max) |>
  mutate('1986' = 0) |>
  ungroup()
...
mod5 <- lm(n ~ . - serial_no, data = alt_regression_df_2)

```

```{r regression_plots}

setDT(as.data.frame(mod5[['coefficients']][50:87]), keep.rownames = TRUE) |>
  rename('effect' = 'mod5[["coefficients"]][50:87]', 'year' = 'rn') |>
  left_join(case_file_filtered_nonword_totals, by = 'year') |>
