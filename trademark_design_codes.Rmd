...
setwd("~/Desktop/Projects/Code/public_trademark_design_codes")
...
case_file_filtered_totals <- case_file_filtered |>
  ...
  mutate(app_total = n()) |>
  ungroup() |>
  filter(!(is.na(publication_dt) | (is.na(registration_dt) & grepl('^0+', registration_no)))) |>
  group_by(year) |>
  mutate(pub_total = n()) |>
...
regression_df <- df_no_codes |>
  ...
  mutate(published = case_when((is.na(publication_dt) == FALSE | (is.na(registration_dt) == FALSE & grepl('^0+', registration_no) == FALSE)) ~ 1 , TRUE ~ 0)) |>
  filter(!grepl('2022|2023|2024', year)) |>
  mutate(bin = case_when(
    grepl('1986|1987|1988|1989|1990', year) == TRUE ~ '1986-1990',
    grepl('1991|1992|1993|1994|1995', year) == TRUE ~ '1991-1995',
    grepl('1996|1997|1998|1999|2000', year) == TRUE ~ '1996-2000',
    grepl('2001|2002|2003|2004|2005', year) == TRUE ~ '2001-2005',
    grepl('2006|2007|2008|2009|2010', year) == TRUE ~ '2006-2010',
    grepl('2011|2012|2013|2014|2015', year) == TRUE ~ '2011-2015',
    grepl('2016|2017|2018|2019|2020', year) == TRUE ~ '2016-2020',
    .default = '2021'
                           )) |>
  mutate(bin_alt = case_when(
    grepl('1987|1988|1989|1990|1991', year) == TRUE ~ '1987-1991',
    grepl('1992|1993|1994|1995|1996', year) == TRUE ~ '1992-1996',
    grepl('1997|1998|1999|2000|2001', year) == TRUE ~ '1997-2001',
    grepl('2002|2003|2004|2005|2006', year) == TRUE ~ '2002-2006',
    grepl('2007|2008|2009|2010|2011', year) == TRUE ~ '2007-2011',
    grepl('2012|2013|2014|2015|2016', year) == TRUE ~ '2012-2016',
    grepl('2017|2018|2019|2020|2021', year) == TRUE ~ '2017-2021',
    .default = '1986'
                           )) |>
  ...
  group_by(serial_no) |>
  mutate(across(c('n', 'n_basic', 'registered', 'published', all_of(intl_classes), 'bin', 'bin_alt', all_of(years), 'attorney'), max)) |>
  filter(!duplicated(serial_no)) |>
  ungroup()
...
regression_df_1 <- regression_df |>
  mutate(highcode = case_when((n > 5) ~ 1, TRUE ~ 0)) |>
  select(serial_no, highcode, published, all_of(intl_classes), bin, attorney)
  
mod1 <- glm(published ~ . - serial_no, family = binomial, data = regression_df_1, trace = FALSE)

mod2 <- glm(published ~ highcode, family = binomial, data = regression_df_1, trace = FALSE)

mod1_cl <- coeftest(mod1, vcov = vcovCL(mod1, cluster = regression_df_1$bin, type = "HC0"))

mod2_cl <- coeftest(mod2, vcov = vcovCL(mod2, cluster = regression_df_1$bin, type = "HC0"))
...
stargazer(mod1, mod2,
          ...
          omit = c(intl_classes, 'attorney'),
          keep = c('highcode', 'bin1986-1990', 'bin1991-1995', 'bin1996-2000', 'bin2001-2005', 'bin2006-2010', 'bin2011-2015', 'bin2016-2020'),
          omit.stat = c("ll","aic"),
          intercept.bottom = TRUE,
          no.space = TRUE,
          column.labels = c('Controls', 'No Controls'),
          report = 'vc*s',
          label = '',
          title = '',
          out = 'table_1.tex')
...
```{r owner_analysis_general}

# get top 10 filers of all marks
...
case_file_filtered |>
  ...
  summarise(n_filed = n()) |>
  ungroup() |>
  slice_max(n = 10, order_by = n_filed)

```

```{r owner_analysis_specific}

# get top 10 filers of all marks

df_no_codes |>
  select(serial_no, mark_draw_cd, filing_dt, n) |>
  filter(n > 5) |>
  left_join(owner, by = 'serial_no') |>
  group_by(serial_no) |>
  arrange(own_type_cd, .by_group = TRUE) |>
...
```{r owner_intl_class_analysis_specific}

# get top international classes in which they file

top_filer_top_classes <- case_file_filtered |>
  select(serial_no, filing_dt) |>
  left_join(owner, by = 'serial_no') |>
  select(own_name, own_type_cd, serial_no, filing_dt) |>
  group_by(serial_no) |>
  arrange(own_type_cd, .by_group = TRUE) |>
  filter(!duplicated(serial_no)) |>
  ungroup() |>
  #filter(own_name == 'Microsoft Corporation' | own_name == 'E. & J. Gallo Winery' | own_name == 'Disney Enterprises, Inc.' | own_name == 'Disney Enterprises, Inc' | own_name == 'Summit Entertainment, LLC' | grepl('Societe.+\\bNestle\\b', own_name)) |>
  filter(own_name == 'Disney Enterprises, Inc.' | own_name == 'Disney Enterprises, Inc' | own_name == 'MATTEL, INC.' | own_name == 'MATTEL INC.' | own_name == 'MATTEL, INC' | own_name == 'MATTEL INC' | own_name == 'MATTEL,INC.' | own_name == 'MATTEL , INC.' | own_name == 'MATTEL, INC,' | own_name == 'HASBRO, INC.' | own_name == 'HASBRO, INC' | own_name == 'HASBRO,. INC.' | own_name == 'HASBRO,INC.' | own_name == 'HASBRO INC.' | own_name == 'HASBRO. INC.' | own_name == 'MATTEL, INC,' | own_name == 'Glaxo Group Limited' | own_name == 'Glaxo Group Ltd.' | own_name == 'Glaxo Group Limited, a British Corporation' | own_name == 'Novartis Ag' | own_name == 'Novartis AG') |>
  left_join(intl_class, by = 'serial_no') |>
...
top_movers_by_intl_class <- case_file_filtered |>
  left_join(intl_class, by = 'serial_no') |>
  group_by(year, intl_class_cd) |>
  mutate(total_general = n()) |>
  ungroup() |>
  left_join(select(df_no_codes, n, serial_no), by = 'serial_no') |>
  select(serial_no, total_general, intl_class_cd, year, n) |>
  filter(n > 5) |>
  group_by(year, intl_class_cd) |>
  mutate(total_highcode = n()) |>
  #mutate(percentage_increase = total_highcode/total_general) |>
  ungroup() |>
  filter(year == 1986 | year == 2023) |>
  group_by(intl_class_cd) |>
  filter(!duplicated(year)) |>
  arrange(year, .by_group = TRUE) |>
  mutate(percentage_change_highcode = ((total_highcode - shift(total_highcode))/shift(total_highcode) * 100) - 100) |>
  mutate(percentage_change_general = ((total_general - shift(total_general))/shift(total_general) * 100) -100) |>
  filter(!duplicated(intl_class_cd)) |>
  ungroup()

```

```{r nonword_depletion_analysis}

depleted_codes_categories_general <- right_join(case_file_filtered, design_search, by = 'serial_no') |>
  filter(grepl('^2+', mark_draw_cd)) |>
  mutate(writing = case_when(grepl('^27+|^28+|^2901+', design_search_cd) == TRUE ~ 1, TRUE ~ 0)) |>
  group_by(serial_no) |>
  mutate(n = n()) |>
  ...
  filter(writing == 0) |>
  mutate(category = str_sub(design_search_cd, 0, -5)) |>
  group_by(design_search_cd) |>
  mutate(n_code_used = n()) |>
  ungroup() |>
  group_by(category) |>
  mutate(n_category_used = n()) |>
  ungroup() |>
  select(serial_no, design_search_cd, category, n_code_used, n_category_used, n)

depleted_codes_categories_highcode <- depleted_codes_categories_general |>
  filter(n > 5) |>
  group_by(design_search_cd) |>
  mutate(n_code_used = n()) |>
  ungroup() |>
...
top_general <- filter(depleted_codes_categories_general, !duplicated(design_search_cd)) |>
  slice_max(n = 10, order_by = n_code_used)

top_highcode <- filter(depleted_codes_categories_highcode, !duplicated(design_search_cd)) |>
  slice_max(n = 10, order_by = n_code_used)

anti_join(top_highcode, top_general, by = 'design_search_cd')

```

```{r nonword_depletion_analysis_by_intl_class_1}

intl_class_pivoted <- df_no_codes |>
  left_join(intl_class, by = 'serial_no') |>
  mutate(intl_class_yes = 1) |>
  pivot_wider(names_from = 'intl_class_cd', values_from = 'intl_class_yes', values_fill = 0) |>
  select('serial_no', all_of(intl_classes)) |>
  group_by(serial_no) |>
  mutate(across(all_of(intl_classes), max)) |>
  filter(!duplicated(serial_no)) |>
  ungroup()

categories <- unique(depleted_codes_categories_general$category)

depleted_codes_categories_intl_class_general <- depleted_codes_categories_general |>
  left_join(intl_class_pivoted, by = 'serial_no') |>
  group_by(design_search_cd) |>
  mutate(across(c(intl_classes), sum)) |>
  filter(!duplicated(design_search_cd)) |>
  ungroup()

depleted_codes_categories_intl_class_highcode <- depleted_codes_categories_highcode |>
  left_join(intl_class_pivoted, by = 'serial_no') |>
  group_by(design_search_cd) |>
  mutate(across(c(intl_classes), sum)) |>
  filter(!duplicated(design_search_cd)) |>
...
```{r nonword_depletion_analysis_by_intl_class_2}

#depleted_codes_categories_intl_class_general <- depleted_codes_categories_intl_class_general |>
  #rename(c9 = '009', c25 = '025', c35 = '035', c41 = '041', c42 = '042')

#depleted_codes_categories_intl_class_highcode <- depleted_codes_categories_intl_class_highcode |>
  #rename(c9 = '009', c25 = '025', c35 = '035', c41 = '041', c42 = '042')

top_depleted_general <- slice_max(depleted_codes_categories_intl_class_general, n = 10, order_by = c9) |>
  rbind(slice_max(depleted_codes_categories_intl_class_general, n = 10, order_by = c25)) |>
  rbind(slice_max(depleted_codes_categories_intl_class_general, n = 10, order_by = c35)) |>
  rbind(slice_max(depleted_codes_categories_intl_class_general, n = 10, order_by = c41)) |>
  rbind(slice_max(depleted_codes_categories_intl_class_general, n = 10, order_by = c42)) |>
  filter(!duplicated(design_search_cd))

top_depleted_highcode <- slice_max(depleted_codes_categories_intl_class_highcode, n = 10, order_by = c9) |>
  rbind(slice_max(depleted_codes_categories_intl_class_highcode, n = 10, order_by = c25)) |>
  rbind(slice_max(depleted_codes_categories_intl_class_highcode, n = 10, order_by = c35)) |>
  rbind(slice_max(depleted_codes_categories_intl_class_highcode, n = 10, order_by = c41)) |>
  rbind(slice_max(depleted_codes_categories_intl_class_highcode, n = 10, order_by = c42)) |>
  filter(!duplicated(design_search_cd))

anti_join(top_depleted_highcode, top_depleted_general, by = 'design_search_cd')

```

```{r codes_vs_app_no_plot}

tikz(file = './graphics/figure_7.tex', width = 6.25, height = 5)

scale1 <- 10^-9

figure_8.1 <- ggplot(super_totals_per_year, aes(year, app_total)) +
  geom_line(aes(color = 'Total Applications'), show.legend = FALSE, linewidth = 1) +
  geom_line(aes(y = percentage10/scale1, color = 'Percentage of Applications with $>$ 10 Codes'), show.legend = FALSE, linewidth = 1) +
...
figure_8.2 <- ggplot(super_totals_per_year, aes(year, app_total)) +
  geom_line(aes(color = 'Total Applications'), show.legend = FALSE, linewidth = 1) +
  geom_line(aes(y = percentage15/scale2, color = 'Percentage of Applications with $>$ 15 Codes'), show.legend = FALSE, linewidth = 1) +
...
```{r log_regression_models_baselining}

regression_df_2 <- regression_df |>
  mutate(highcode = case_when((n > 5) ~ 1, TRUE ~ 0)) |>
  select(serial_no, highcode, all_of(intl_classes), bin_alt, attorney, n_basic)
  
mod3 <- glm(highcode ~ . - serial_no - n_basic, family = binomial, data = regression_df_2, trace = FALSE)

mod4 <- glm(highcode ~ bin_alt, family = binomial, data = regression_df_2, trace = FALSE)

mod5 <- glm(highcode ~ . - serial_no, family = binomial, data = regression_df_2, trace = FALSE)

mod3_cl <- coeftest(mod3, vcov = vcovCL(mod3, cluster = regression_df_2$bin_alt, type = "HC0"))

mod4_cl <- coeftest(mod4, vcov = vcovCL(mod4, cluster = regression_df_2$bin_alt, type = "HC0"))

mod5_cl <- coeftest(mod5, vcov = vcovCL(mod5, cluster = regression_df_2$bin_alt, type = "HC0"))
...
figure_8.1 <- tibble(c(unique(sort(regression_df_2$bin_alt))), c(0, mod3_cl[50:56])) |>
  rename('x' = 'c(unique(sort(regression_df_2$bin_alt)))') |>
  rename('y' = 'c(0, mod3_cl[50:56])') |>
  ggplot(aes(x, y)) +
  geom_point() +
  labs(x = '5-Year Period', y = 'Estimated Effect') +
  theme(axis.title.x = element_text(vjust = -3, size = 7.5),
        axis.title.y = element_text(vjust = 3, size = 7.5),
        axis.text.x = element_text(vjust = 0.375, angle = 45, size = 6),
        axis.text.y = element_text(size = 6),
        plot.title = element_text(size = 9)) +
  ggtitle('Control for representation only')

figure_8.2 <- tibble(c(unique(sort(regression_df_2$bin_alt))), c(0, mod5_cl[50:56])) |>
  rename('x' = 'c(unique(sort(regression_df_2$bin_alt)))') |>
  rename('y' = 'c(0, mod5_cl[50:56])') |>
  ggplot(aes(x, y)) +
  geom_point() +
  labs(x = '', y = '') +
  theme(axis.title.x = element_text(vjust = -3, size = 7.5),
        ...
        axis.text.y = element_text(size = 6),
        plot.title = element_text(size = 9)) +
  ggtitle('Control for representation and number of basic codes')
...
alt_regression_df_2 |>
  ...
  labs(y = 'Number of Applications with $>$ 5 Codes', x = 'International Class')
